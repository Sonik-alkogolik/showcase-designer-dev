...
- `GIT_RELEASE_WORKFLOW.md` = регламент разработки и релиза между `origin` (dev) и `prod` (production).
...
14. Домен и фокус работ (2026-03-29):
   - Подтверждено, что `https://ec-dn.ru` — WordPress-витрина, не backend Laravel API для checkout smoke.
   - Куплен новый домен `e-tgo.ru` для переноса сервиса.
   - В коде убраны критичные хардкоды домена:
     - `config/cors.php` теперь берёт origin из `FRONTEND_URL`/`APP_URL`;
     - `telegram:set-webhook` больше не использует хардкод URL, берёт аргумент или `TELEGRAM_WEBHOOK_URL`;
...
17. Telegram/API access stabilization confirmed (2026-04-10):
   - Подтверждено, что доступ и webhook-контур работают стабильно, сервис в рабочем состоянии.
   - Риски из предыдущего handoff про нестабильный egress к `api.telegram.org` и "ручные обходы" больше не считаются актуальным блокером.
   - Фокус команды возвращён на продуктовый проход core-flow, без инфраструктурной аварийной ветки.

18. Telegram avatar in profile + mini-app confirmed on production (2026-04-10):
   - Деплой обновлён до актуального `main` и применена миграция `telegram_avatar_url`.
...
1. Блокирующих инфраструктурных рисков по Telegram/webhook на текущем этапе не зафиксировано.
2. Главный рабочий риск смещён в продуктовую плоскость: возможные дефекты в полном ручном core-flow.
